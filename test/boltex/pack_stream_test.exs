defmodule Boltex.PackStreamTest do
  ...
  describe "Encodes messages:" do
    test "ack_failure" do
      assert <<0x0, 0x2, 0xB0, 0xE, 0x0, 0x0>> = PackStream.Message.encode(:ack_failure)
    end

    test "discard_all" do
      assert <<0x0, 0x2, 0xB0, 0x2F, 0x0, 0x0>> = PackStream.Message.encode(:discard_all)
    end

    test "init" do
      assert <<0x0, 0x10, 0xB2, _::binary>> = PackStream.Message.encode(:init)
      ...
      assert <<0x0, 0x42, 0xB2, 0x1, 0x8C, 0x42, 0x6F, 0x6C, 0x74, 0x65, 0x78, 0x2F, 0x30, 0x2E,
               ...
               0x0>> = PackStream.Message.encode(:init, {"neo4j", "password"})
    end

    test "pull_all" do
      assert <<0x0, 0x2, 0xB0, 0x3F, 0x0, 0x0>> = PackStream.Message.encode(:pull_all)
    end

    test "reset" do
      assert <<0x0, 0x2, 0xB0, 0xF, 0x0, 0x0>> = PackStream.Message.encode(:reset)
    end

    test "run" do
      assert <<0x0, 0x13, 0xB2, 0x10, 0x8F, 0x52, 0x45, 0x54, 0x55, 0x52, 0x4E, 0x20, 0x31, 0x20,
               0x41, 0x53, 0x20, 0x6E, 0x75, 0x6D, 0xA0, 0x0,
               0x0>> = PackStream.Message.encode(:run, "RETURN 1 AS num")
      ...
      <<0x0, 0x1D, 0xB2, 0x10, 0xD0, 0x13, 0x52, 0x45, 0x54, 0x55, 0x52, 0x4E, 0x20, 0x7B, 0x6E,
        ...
        0x5, 0x0, 0x0>> = PackStream.Message.encode(:run, "RETURN {num} AS num", %{num: 5})
    end
  end
